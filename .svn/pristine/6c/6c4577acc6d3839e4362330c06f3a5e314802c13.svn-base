...
//    char* old_buf = NULL;
    ...
    if((buf = readline("Please enter intersection name: ")) != NULL) { //Prompt the user for input
        ...
        for (unsigned i = 0; i < getNumberOfIntersections(); i++){
            if (intersection_names[i].name == (static_cast<string>(buf)).substr(1,(static_cast<string>(buf)).length()-3)) //truncate the quotation marks from the string
                intersection_id = i;
        }
...
char* intersection_name_generator(const char* stem_text, int state) {
    ...
    while(count < static_cast<int>(intersection_names.size()) - 1) {
        count++;
