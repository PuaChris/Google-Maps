...
// Sets up unordered map for POI lookups
...
//    std::cout << "find_path_between_intersections(): " << delta_time.count() << "ns" << std::endl;
//    std::cout << "distance: " << find_distance_between_two_points(getIntersectionPosition(intersect_id_start), getIntersectionPosition(intersect_id_end)) << std::endl;
    ...
    while (!wavefront.empty()) {
        ...
        //if not, check all adjacent intersections
            ...
            for (size_t i = 0; i < num_edges; i++) {
                ...
                if (static_cast<int>(edge_id) == curr->reachingEdge) {
                    continue;
                }
...
//                    toNode->total_time = new_travel_time;
...
std::vector<unsigned> bfsTraceBack(int destID) {
    ...
    int prevEdgeID = currNode->reachingEdge;
