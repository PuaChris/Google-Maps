...
bool isLegal(Path& path_inter){
    ...
    for(int i = 0; i < size; i++){
        ...
        else if (current_point.deliveryID != DEPOT_ID){
            auto it = pickUps.find(current_point.deliveryID);
            if(it == pickUps.end()) {
//                std::cout << "illegal bc drop off before pick up!" << std::endl;
                return false;  
            }
            //if pickUp not found, path is illegal 
            if(it->second == true){
//                std::cout << "illegal bc package already dropped off!" << std::endl;
                return false; 
            }           //if package already dropped off, path is illegal
            it->second = true;                             //if pick up found, mark it as dropped off
        }
            
    }
  
    if (pickUps.size() != path_inter.pathSize()/2 - 1){
//        std::cout << "illegal bc size doesnt match!" << std::endl;
        return false;
    } //not all packages were picked up
...
//perturbs order of deliveries locally
Path & local_perturbation(Path& path_inter){
    
    unsigned improve = 0;

    while (improve < path_inter.pathSize() - 4) {
        //go thru entire path
        for (int i = 2; i < path_inter.pathSize() - 3; i++) {
            
            std::cout<< "at i =  " << i << std::endl;
            
            //check if swap is legal, only potentially illegal case would be swapping pickup and then drop off
            if (path_inter.path[i].pickUp == true && path_inter.path[i + 1].pickUp == false) {
                
                if (isAlreadyPickedUp(i, path_inter.path[i + 1].deliveryID, path_inter) == false){
                    continue;
                    std::cout<< "Swap is illegal" << std::endl;
                }
            }

            //check if swap is advantageous
            if (!isAdvantageous(i,path_inter.path)) {
                std::cout<< "Swap is slower" << std::endl;
                continue;
            }
            //if so, swap
            path_inter.swap(i, i + 1);
            improve = 0;
            std::cout<< "Swaped " << i << " and " << i+1 << std::endl;
        }
        improve ++;
    }
...
//given a dropOff, and its index in the path, check if the packaged is already picked up
bool isAlreadyPickedUp(unsigned dropOffIndex, unsigned deliveryID_, Path& path_inter){
    
    for (int i = 1; i < dropOffIndex; i++){
        if (path_inter.path[i].deliveryID == deliveryID_ && path_inter.path[i].pickUp)
            return true;
    }
    return false;
}

bool isAdvantageous(unsigned i, std::vector<DeliveryPoint>& path){
    
    double old_distance = find_distance_btw_delivery_points(path[i-1],path[i]) + find_distance_btw_delivery_points(path[i+1],path[i+2]);
    double new_distance = find_distance_btw_delivery_points(path[i-1],path[i+1]) + find_distance_btw_delivery_points(path[i],path[i+2]);
